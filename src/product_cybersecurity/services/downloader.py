...
class DownloadService:
    ...
    def extract_cves(self, years: Optional[int] = None) -> Path:
        ...
        if not zip_path.exists():
            raise FileNotFoundError(f"CVE zip not found at {zip_path}. Run download_cves() first.")
        ...
        with zipfile.ZipFile(zip_path, 'r') as z:
            ...
            extracted_count = 0
            iterator = tqdm(json_members, desc="Extracting", unit="file") if not self.quiet else json_members
